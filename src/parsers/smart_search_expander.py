"""
Модуль для расширения поисковых запросов с помощью LLM.
Генерирует синонимы и связанные термины для поиска тендеров.
"""

from typing import List, Dict, Any
import json
import sys
from pathlib import Path

# Добавляем путь к корневой директории
sys.path.insert(0, str(Path(__file__).parent.parent))

try:
    from utils.transliterator import Transliterator
except ImportError:
    # Fallback если не можем импортировать
    class Transliterator:
        @classmethod
        def has_latin(cls, text):
            import re
            return bool(re.search(r'[a-zA-Z]', text))

        @classmethod
        def transliterate(cls, text):
            return text

        @classmethod
        def generate_variants(cls, text):
            return [text]


class SmartSearchExpander:
    """Расширяет поисковые запросы для более широкого охвата."""

    def __init__(self, llm_adapter):
        """
        Инициализация расширителя поиска.

        Args:
            llm_adapter: Адаптер для работы с LLM
        """
        self.llm = llm_adapter
        self.transliterator = Transliterator()

    def expand_search_query(self, original_query: str, max_variants: int = 5) -> List[str]:
        """
        Генерирует связанные поисковые запросы.

        Args:
            original_query: Оригинальный запрос (например, "компьютерное оборудование")
            max_variants: Максимальное количество вариантов

        Returns:
            Список поисковых запросов (включая оригинал)
        """
        # Проверяем наличие латиницы
        has_latin = self.transliterator.has_latin(original_query)

        # Дополнительные инструкции для латинских запросов
        latin_instructions = ""
        if has_latin:
            transliterated = self.transliterator.transliterate(original_query)
            latin_instructions = f"""
⚠️ ВАЖНО: Запрос содержит латинские символы (названия брендов).
Zakupki.gov.ru RSS API НЕ ПОДДЕРЖИВАЕТ латиницу, поэтому:
1. ОБЯЗАТЕЛЬНО включи кириллическую версию: "{transliterated}"
2. Генерируй варианты БЕЗ названия бренда (только категория товара)
3. Используй ТОЛЬКО кириллицу в вариантах
4. НЕ включай латинские символы в расширенные варианты

Примеры ПРАВИЛЬНЫХ вариантов для "Atlas Copco":
✅ "Атлас Копко"
✅ "компрессоры винтовые"
✅ "пневматическое оборудование"
✅ "компрессорное оборудование"
✅ "воздушные компрессоры"

Примеры НЕПРАВИЛЬНЫХ вариантов:
❌ "компрессоры Atlas Copco" (содержит латиницу!)
❌ "Atlas Copco оборудование" (содержит латиницу!)
"""

        prompt = f"""Ты - эксперт по государственным закупкам в России.

ЗАДАЧА: Сгенерируй список связанных поисковых запросов для поиска тендеров на zakupki.gov.ru.

ОРИГИНАЛЬНЫЙ ЗАПРОС: "{original_query}"
{latin_instructions}

ТРЕБОВАНИЯ:
1. Включи синонимы и близкие термины
2. Включи конкретные категории товаров/услуг, которые входят в это понятие
3. Включи распространенные формулировки из реальных тендеров
4. Используй терминологию госзакупок
5. Верни {max_variants} наиболее релевантных вариантов
6. ИСПОЛЬЗУЙ ТОЛЬКО КИРИЛЛИЦУ в вариантах (без латинских символов!)

ПРИМЕРЫ:
- Для "компьютерное оборудование": видеокарты, процессоры, комплектующие для компьютеров, периферийные устройства, системные блоки
- Для "программное обеспечение": лицензии на ПО, антивирусное ПО, офисные приложения, специализированное ПО

Верни результат СТРОГО в JSON формате:
{{
  "original": "оригинальный запрос",
  "expanded": [
    "вариант 1",
    "вариант 2",
    "вариант 3"
  ],
  "reasoning": "краткое объяснение почему выбраны эти варианты"
}}
"""

        try:
            response = self.llm.generate(system_prompt="Ты - эксперт по государственным закупкам в России.", user_prompt=prompt)

            # Парсим JSON из ответа
            # Ищем JSON в ответе
            start_idx = response.find('{')
            end_idx = response.rfind('}') + 1

            if start_idx != -1 and end_idx > start_idx:
                json_str = response[start_idx:end_idx]
                result = json.loads(json_str)

                # Формируем список запросов
                queries = []

                # Добавляем транслитерированную версию если есть латиница
                if has_latin:
                    transliterated = self.transliterator.transliterate(original_query)
                    queries.append(transliterated)
                    print(f"\n💡 Расширение запроса '{original_query}':")
                    print(f"   🔄 Транслитерация: '{transliterated}'")
                else:
                    queries.append(original_query)
                    print(f"\n💡 Расширение запроса '{original_query}':")

                if 'expanded' in result and isinstance(result['expanded'], list):
                    # Фильтруем варианты, убираем те что содержат латиницу
                    filtered_variants = []
                    skipped_variants = []

                    for variant in result['expanded'][:max_variants * 2]:  # Берем с запасом
                        if self.transliterator.has_latin(variant):
                            skipped_variants.append(variant)
                        else:
                            filtered_variants.append(variant)

                        # Останавливаемся когда набрали нужное количество
                        if len(filtered_variants) >= max_variants - 1:
                            break

                    queries.extend(filtered_variants[:max_variants - 1])

                    print(f"   📋 Сгенерировано вариантов: {len(queries) - 1}")
                    for i, q in enumerate(queries[1:], 1):
                        print(f"      {i}. {q}")

                    if skipped_variants:
                        print(f"   ⚠️  Пропущено вариантов с латиницей: {len(skipped_variants)}")
                        for variant in skipped_variants[:3]:
                            print(f"      ❌ {variant}")

                if 'reasoning' in result:
                    print(f"   🤔 Обоснование: {result['reasoning']}")

                return queries

        except Exception as e:
            print(f"⚠️  Ошибка расширения запроса через LLM: {e}")

        # Возвращаем транслитерированную версию если есть латиница
        if has_latin:
            transliterated = self.transliterator.transliterate(original_query)
            return [transliterated]

        return [original_query]

    def generate_okpd_categories(self, query: str) -> List[str]:
        """
        Генерирует возможные коды ОКПД2 для запроса.

        Args:
            query: Поисковый запрос

        Returns:
            Список кодов ОКПД2
        """
        prompt = f"""Определи возможные коды ОКПД2 (Общероссийский классификатор продукции) для запроса: "{query}"

Верни 3-5 наиболее релевантных кодов ОКПД2 в формате JSON:
{{
  "codes": [
    {{"code": "26.20.1", "name": "Компьютеры и периферийное оборудование"}},
    {{"code": "26.30.5", "name": "Оборудование коммуникационное"}}
  ]
}}
"""

        try:
            response = self.llm.generate(system_prompt="Ты - эксперт по государственным закупкам в России.", user_prompt=prompt)

            start_idx = response.find('{')
            end_idx = response.rfind('}') + 1

            if start_idx != -1 and end_idx > start_idx:
                json_str = response[start_idx:end_idx]
                result = json.loads(json_str)

                if 'codes' in result:
                    return [item['code'] for item in result['codes']]

        except Exception as e:
            print(f"⚠️  Ошибка определения ОКПД2: {e}")

        return []


class TenderDataExtractor:
    """Извлекает детальные данные о тендере с помощью LLM."""

    def __init__(self, llm_adapter):
        """
        Инициализация экстрактора данных.

        Args:
            llm_adapter: Адаптер для работы с LLM
        """
        self.llm = llm_adapter

    def extract_tender_details(self, tender_summary: str) -> Dict[str, Any]:
        """
        Извлекает детальную информацию из описания тендера.

        Args:
            tender_summary: Описание тендера из RSS или HTML

        Returns:
            Словарь с извлеченными данными
        """
        prompt = f"""Проанализируй описание тендера и извлеки структурированную информацию.

ОПИСАНИЕ ТЕНДЕРА:
{tender_summary[:3000]}

ЗАДАЧА: Извлеки следующую информацию и верни в JSON формате:

{{
  "tender_type": "Тип закупки (товары/работы/услуги)",
  "categories": ["Категория 1", "Категория 2"],
  "region": "Регион",
  "customer_type": "Тип заказчика (федеральный/региональный/муниципальный)",
  "submission_deadline": "Срок подачи заявок (дата и время)",
  "winner_determination_date": "Срок определения победителя (дата и время)",
  "payment_terms": "Условия оплаты (если указаны)",
  "quantity_info": "Информация о количестве товаров (если указано)",
  "okpd_codes": ["Код ОКПД2 если есть"]
}}

ВАЖНО:
- Если информация не найдена, используй null
- Для categories укажи все упомянутые категории товаров/услуг
- region - название региона из текста
- submission_deadline - точная дата и время окончания приема заявок
- winner_determination_date - дата подведения итогов, определения победителя
- payment_terms - любая информация об аванс, рассрочке, порядке оплаты
"""

        try:
            response = self.llm.generate(system_prompt="Ты - эксперт по государственным закупкам в России.", user_prompt=prompt)

            start_idx = response.find('{')
            end_idx = response.rfind('}') + 1

            if start_idx != -1 and end_idx > start_idx:
                json_str = response[start_idx:end_idx]
                result = json.loads(json_str)
                return result

        except Exception as e:
            print(f"   ⚠️  Ошибка извлечения данных через LLM: {e}")

        # Возвращаем пустой результат
        return {
            "tender_type": None,
            "categories": [],
            "region": None,
            "customer_type": None,
            "submission_deadline": None,
            "winner_determination_date": None,
            "payment_terms": None,
            "quantity_info": None,
            "okpd_codes": []
        }


def main():
    """Пример использования."""
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).parent.parent))

    from analyzers.tender_analyzer import TenderAnalyzer
    from utils.config_loader import ConfigLoader

    print("\n" + "="*70)
    print("  ТЕСТ УМНОГО РАСШИРЕНИЯ ПОИСКА")
    print("="*70 + "\n")

    # Загружаем конфигурацию
    config = ConfigLoader()
    llm_config = config.get_llm_config()

    # Создаем TenderAnalyzer
    tender_analyzer = TenderAnalyzer(
        api_key=llm_config['api_key'],
        provider=llm_config['provider'],
        model_fast=llm_config.get('model_fast')
    )

    # Создаем расширитель
    expander = SmartSearchExpander(tender_analyzer.llm)

    # Тестируем расширение запросов
    test_queries = [
        "компьютерное оборудование",
        "программное обеспечение",
        "канцелярские товары"
    ]

    for query in test_queries:
        print(f"\n{'─'*70}")
        print(f"Оригинальный запрос: '{query}'")
        print(f"{'─'*70}")

        expanded = expander.expand_search_query(query, max_variants=5)

        print(f"\n✅ Результат расширения ({len(expanded)} запросов):")
        for i, q in enumerate(expanded, 1):
            print(f"   {i}. {q}")

    print("\n" + "="*70)


if __name__ == "__main__":
    main()
