...
class SmartMatcher:
    ...
    def _match_tender_internal(
        ...
        if keywords:
            ...
            # ШАГ 6.5: Штраф за одно совпадение при малом числе ключевых слов
            # Для фильтров с 3-4 словами (тематические, напр. "разработка ПС сервис") — 1 совпадение слишком мало.
            # Для фильтров с 5+ словами (каталожные, напр. "холодильник стиральная машина ...") — 1 совпадение нормально.
            if total_criteria in (3, 4) and len(matched_keywords) == 1:
                score = int(score * 0.3)  # сильный штраф вместо отсечения
                logger.debug(f"   ⚠️ Только 1 совпадение из {total_criteria} — штраф, score={score}")
