    ...
    def match_tender(
        ...
        if regions and tender_region:
            ...
            for region in expanded_regions:
                region_lower = region.lower()
                # Проверяем вхождение в обе стороны (для вариативности написания)
                if region_lower in tender_region or tender_region in region_lower:
                    region_match = True
                    break

            if not region_match:
                logger.debug(f"   ⛔ Регион не подходит: {tender_region} (ожидалось: {expanded_regions[:5]}...)")
