...
class MultiStageAnalyzer:
    ...
    def _extract_basic_info(self, documentation: str) -> dict:
        ...
        """
        # Извлекаем разделы с ключевой информацией
        # НМЦК может быть в извещении, проекте контракта, титульном листе
        key_section = self.truncator.extract_section_by_keyword(
            documentation,
            keywords=["начальная максимальная цена", "нмцк", "цена контракта", "обеспечение контракта", "обеспечение заявки", "извещение"],
            max_chars=25000
        )

        # Если не нашли ключевые разделы, берем начало документа
        if not key_section or len(key_section) < 500:
            key_section = documentation[:25000]
...
ВАЖНЫЕ ИНСТРУКЦИИ ПО ИЗВЛЕЧЕНИЮ НМЦК И ОБЕСПЕЧЕНИЙ:
1. НМЦК ищи по ключевым словам: "начальная максимальная цена контракта", "НМЦК", "цена контракта"
2. Обеспечение контракта ищи: "обеспечение исполнения контракта", "размер обеспечения"
3. Обеспечение заявки ищи: "обеспечение заявки на участие"
4. КРИТИЧНО: Если видишь число с "руб" или "рублей" рядом с этими словами - это и есть искомое значение!
...
- nmck: Начальная максимальная цена контракта (ТОЛЬКО ЧИСЛО в рублях, без текста)
...
- application_guarantee: Обеспечение заявки на участие (число в рублях или % от НМЦК)
- contract_guarantee: Обеспечение исполнения контракта (число в рублях или % от НМЦК)
...
{{
  ...
  "application_guarantee": "5% или число в рублях",
...
- "НМЦК: 1 500 000,00 руб." → nmck: 1500000
- "Обеспечение заявки: 5%" → application_guarantee: "5%"
- "Обеспечение контракта: 75 000 руб." → contract_guarantee: "75000"
...
КРИТИЧЕСКИ ВАЖНО для specifications:
- ВСЕГДА извлекай ЧИСЛОВЫЕ ЗНАЧЕНИЯ характеристик
- НЕ пиши просто "высота каркаса" - пиши "высота каркаса: 1350-1550 мм"
- НЕ пиши просто "материал" - пиши "материал: алюминий"
- НЕ пиши просто "нагрузка" - пиши "максимальная нагрузка: 135 кг"
- Если в таблице указано "Высота каркаса, мм: 1350-1550" - извлекай КАК "высота каркаса": "1350-1550 мм"
- Если указано "Материал: алюминий/сталь" - извлекай КАК "материал": "алюминий/сталь"

ПРИМЕРЫ ПРАВИЛЬНОГО ИЗВЛЕЧЕНИЯ:
❌ НЕПРАВИЛЬНО: "высота": true или "высота": "есть"
✅ ПРАВИЛЬНО: "высота": "1350-1550 мм"

❌ НЕПРАВИЛЬНО: "регулировка высоты": "есть"
✅ ПРАВИЛЬНО: "регулировка высоты": "110-135 см с шагом 2.5 см"

❌ НЕПРАВИЛЬНО: "материал каркаса": "металл"
✅ ПРАВИЛЬНО: "материал каркаса": "алюминиевый сплав"

ВАЖНО:
- Извлекай ВСЕ позиции, даже если их много
- Включай все упомянутые характеристики С ИХ КОНКРЕТНЫМИ ЗНАЧЕНИЯМИ
