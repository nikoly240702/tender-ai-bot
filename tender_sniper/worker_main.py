...
async def main():
    ...
    if not is_tender_sniper_enabled():
        ...
        try:
            ...
            #
            # ОТКЛЮЧЁН 22.09.2026 по решению владельца: «там объективно нечего
            # ловить — очень низкие НМЦК, которые опускают ещё ниже в ходе
            # торгов». Решение основано на опыте участия, а не на наших
            # данных: по объявлениям Портал выглядел не хуже основного потока
            # (медиана НМЦК 398 тыс. против 291 тыс. у ЕИС, мелочи до 300 тыс.
            # 37% против 51%), но результатов торгов по нему мы не собираем —
            # проверить вторую половину довода нечем.
            #
            # Код и данные остаются на месте: чтобы вернуть, достаточно
            # MOS_PORTAL_ENABLED=1 в окружении.
            if os.getenv("MOS_PORTAL_ENABLED", "").strip() in ("1", "true", "yes"):
                mos_portal_task = asyncio.create_task(mos_portal_poll_loop())
                logger.info("✅ Портал поставщиков включён через MOS_PORTAL_ENABLED")
            else:
                logger.info("⏸️  Портал поставщиков отключён (решение 22.09.2026)")
