...
def _to_api_timestamp(dt: datetime) -> str:
    """Московское время БЕЗ суффикса таймзоны — формат, который реально
    понимает API Портала поставщиков.

    История (важно, легко сломать обратно):
      1) Сначала слали ISO с оффсетом '+03:00' — API отвечал 400: шлюз
         декодирует '%2B' в '+', а затем ещё раз трактует '+' как пробел,
         и до парсера доходит "23:09:26 03:00".
      2) Тогда перешли на UTC с 'Z' — 400 ушёл, но появился тихий сдвиг:
         API трактует присланное время как МОСКОВСКОЕ, поэтому окно
         постоянно отставало на 3 часа и в рабочие часы не находило
         ничего (сутки подряд «тендеров 0» при 593 реально
         опубликованных — замерено 15.09.2026).
      3) Итог: шлём MSK без таймзоны. Проверено на живом API — один и тот
         же момент: UTC+Z даёт count=0, MSK-naive даёт count=230.
    """
    return dt.astimezone(MOSCOW_TZ).strftime("%Y-%m-%dT%H:%M:%S")
...
async def mos_portal_poll_loop():
    ...
    while True:
        try:
            ...
            # "Сейчас" в московском времени: API — московский и трактует
            # присланное время как MSK (см. _to_api_timestamp).
