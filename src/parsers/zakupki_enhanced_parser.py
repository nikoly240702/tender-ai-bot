    ...
    def search_with_details(
        ...
        while len(enhanced_tenders) < max_results and attempt < max_attempts:
            ...
            if not tenders:
                ...
                if attempt == 1:
                    print(f"\n❌ Не удалось получить данные с zakupki.gov.ru")
                    print(f"   Проверьте:")
                    print(f"   • Доступность сайта zakupki.gov.ru")
                    print(f"   • Работоспособность прокси (если используется)")
                    print(f"   • Настройки PROXY_URL в .env файле\n")
                break
