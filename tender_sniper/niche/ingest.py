...
# Все субъекты РФ кодами КЛАДР: 01–79 сплошь, дальше с пропусками —
# 80, 81, 82, 84, 85, 88 не заняты (упразднённые автономные округа),
# а 83, 86, 87, 89 и 91, 92 заняты.
#
# Стоимость охвата: замер 21.09.2026 — один регион-день это около 28
# секунд (девять источников: четыре типа извещений, четыре протокола и
# контракты). По всей стране сутки данных грузятся примерно 40 минут,
# что для ночной догрузки нормально. А вот исторический период так не
# наберёшь: полгода на 85 регионов — это месяцы непрерывной работы,
# историю приходится добирать регионами по мере надобности.
RF_REGIONS = ([f"{n:02d}" for n in range(1, 80)]
              + ["83", "86", "87", "89", "91", "92"])

REGION_GROUPS = {"ЦФО": CFO_REGIONS, "CFO": CFO_REGIONS,
                 "РФ": RF_REGIONS, "RF": RF_REGIONS}
