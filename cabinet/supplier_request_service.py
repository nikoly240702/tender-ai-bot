...
ВХОД:
1. Текст ТЗ заказчика — ОФИЦИАЛЬНЫЙ ДОКУМЕНТ из тендера. Содержит наименования, характеристики, количества.
2. JSON-массив товаров из нашего каталога (id, наименование, размеры, параметры, цена за единицу).

ЗАДАЧА: извлечь из ТЗ РЕАЛЬНЫЕ позиции (которые ЯВНО упомянуты в тексте) и для каждой найти лучший matching-товар из каталога.
...
- Сохрани РЕАЛЬНЫЕ характеристики товара: название, размеры, параметры, объём, материал, цвет, упаковка
- Сохрани количество если оно есть в тексте
- Структурируй как нумерованный список позиций

ЖЁСТКИЕ ПРАВИЛА:
1. Используй ТОЛЬКО данные которые БУКВАЛЬНО есть в тексте ТЗ.
2. НИКОГДА не выдумывай позиции, размеры, материалы, количества. Если в тексте нет конкретных характеристик — НЕ ПРИДУМЫВАЙ.
3. Если в тексте ТЗ только название тендера без позиций (например «Поставка медицинских изделий» и больше ничего) — верни ровно одну строку:
   «НЕДОСТАТОЧНО ДАННЫХ: в ТЗ нет конкретных позиций с характеристиками. Загрузите полную документацию тендера в карточку.»

Формат если данные есть — нумерованный список:
1. <Название>. <Характеристики из ТЗ>. Кол-во: N <ед.> (если есть в ТЗ).
2. ...
