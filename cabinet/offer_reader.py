...
_QUERY_PROMPT = """Подбери поисковый запрос, по которому этот товар ищут в каталогах поставщиков.

ТРЕБОВАНИЕ ИЗ ТЕНДЕРА: {position}

ПРАВИЛА:
- Пиши так, как товар называет продавец, а не заказчик. «Светильник светодиодный внутреннего освещения настенно-потолочный квадратный 500-600 мм» продаётся как «светильник светодиодный 595х595 накладной» — это стандартный типоразмер, и именно он стоит в каталогах.
- Убери канцелярит: «внутреннего освещения», «для нужд», «наличие», «тип», «требования к оснащению».
- Оставь общепринятое торговое обозначение, если оно есть: типоразмер, формат, стандарт, маркировку.
- Оставь 1-2 ключевых числа с единицами — те, по которым товар отличают (мощность, объём, диагональ). Лишние числа сужают выдачу до нуля.
- Не добавляй слов «купить», «цена», «оптом» — они подставляются отдельно.
- 4-8 слов. Ничего не выдумывай: если торгового обозначения не знаешь, просто назови товар и главное число.

Ответь строго JSON: {{"query": "поисковый запрос"}}"""


async def suggest_query(position: str) -> Optional[str]:
    """Запрос к поисковику на языке каталога, а не тендера.

    Замер 21.09.2026 по закупке 0373100128326000093 показал, что это и
    есть главный шаг, который отличал ручной поиск от автоматического:
    «настенно-потолочный квадратный, длина не менее 500 и менее 600 мм»
    вручную превращалось в «595х595 Армстронг» и находило нужный
    светильник за 1 179–1 344 ₽, а дословная формулировка из извещения
    не находила ничего. Перевода строковыми правилами не сделать —
    стандартный типоразмер из диапазона не выводится.

    None, если модель недоступна: вызывающий откатывается на запрос из
    слов самой позиции.
    """
    api_key = os.getenv('OPENAI_API_KEY')
    if not api_key or not position:
    ...
    def _call():
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {'role': 'system',
                 'content': 'Ты подбираешь поисковые запросы по каталогам '
                            'поставщиков. Только JSON.'},
                {'role': 'user',
                 'content': _QUERY_PROMPT.format(position=position[:400])},
            ],
        )
        return resp.choices[0].message.content
    ...
    try:
        ...
        logger.warning(f'Не удалось подобрать запрос: {e}')
        return None
