...
# Слова, задающие оптовое намерение. Раньше здесь было «купить цена», и
# это оказалось прямой причиной бесполезной выдачи: розничное намерение
# поднимает маркетплейсы, а их мы читать не можем — отвечают капчей.
#
# Замер 21.09.2026 на живых запросах:
#   «бахилы водонепроницаемые купить цена»  → 73% маркетплейсов
#   «бахилы водонепроницаемые оптом прайс»  →  0%
#   «шприц одноразовый 10 мл купить цена»   → 30%
#   «шприц одноразовый 10 мл оптом прайс»   →  0%
# Во втором случае в выдаче настоящие оптовики: mirbahil.ru,
# kometa-med.com, biomedtorg.ru, dealmed.ru.
#
# Задачу исходного «купить» эти слова решают не хуже: сами тендеры и
# нормативка в выдачу с ними тоже не лезут.
WHOLESALE_INTENT = "оптом прайс поставщик"

# Потолок длины запроса в словах. Длинные запросы Яндекс отрабатывает
# хуже: чем больше слов, тем меньше документов содержит их все. Считаем
# вместе со словами намерения — иначе их добавление незаметно удлиняет
# запрос сверх потолка.
MAX_QUERY_WORDS = 12
...
def build_query(position: str) -> str:
    ...
    Намерение оптовое, а не розничное: мы ищем, у кого закупиться под
