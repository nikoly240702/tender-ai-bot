...
ПРИНЦИП ОЦЕНКИ:
- Лучше показать пользователю лишний тендер, чем пропустить нужный
- При сомнениях — отвечай "релевантен" с низким confidence
- Отклоняй ТОЛЬКО если тендер СОВЕРШЕННО не связан с запросом
- Даже косвенно связанные тендеры — помечай как релевантные с confidence 30-50%

ПОДСКАЗКИ:
- "разработка" в контексте строительства (проектная документация, месторождения) — не IT
- "система" может быть не IT (пожарная, отопления, охраны)
- Но если есть хоть малейшая связь с запросом — считай релевантным
